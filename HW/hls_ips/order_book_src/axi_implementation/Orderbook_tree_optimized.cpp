...
void Orderbook_wrapper(
		...
		hls::stream<axis_bit_t >& vio_publish_order,
		hls::stream<axis_word_t >& outStream_algo
) {
...
#pragma HLS array_partition variable=g_orderbook.ask_books complete dim=1
    ...
    if (initialized && !publish_order.empty())
    {
    	axis_bit_t publish_order_request = publish_order.read();
    	publish(outStream_algo);
    }
    if (initialized && !vio_publish_order.empty())
	{
		axis_bit_t publish_order_request = publish_order.read();
		publish(outStream_algo);
	}

//    else if (!publish_order.empty() && publish_order_request.data == 0 && pub_deb == true)
//    {
//        pub_deb = false;
//    }
//
//    // Publish Order block
//    if (initialized && send_counter > 0)
//    {
//    	publish(outStream_algo);
//        send_counter--;
//    }
    ...
    // Parser Data Block
	if (initialized && !inStream_pars.empty()) {
